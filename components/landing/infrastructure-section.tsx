...
const regions = [
  { name: "WhatsApp", nodes: 180, status: "available" },
  { name: "Email", nodes: 240, status: "available" },
  { name: "SMS", nodes: 200, status: "available" },
  { name: "Monde entier", nodes: 195, status: "available" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              <h2 className={`text-6xl md:text-7xl lg:text-[128px] font-display tracking-tight leading-[0.9] transition-all duration-1000 ${
                ...
                Touchez-les
                <br />
                <span className="text-muted-foreground">partout.</span>
              </h2>

              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                isVisible ? "opacity-100" : "opacity-0"
              }`}>
                Deliver WhatsApp, Email, and SMS campaigns to recipients in 195 countries.
                Une plateforme pour chaque contact, partout dans le monde.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-8xl lg:text-[10rem] font-display leading-none">195</span>
                <span className="text-2xl text-muted-foreground">countries</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                Carrier-grade routing across the globe for fast, reliable delivery on every channel.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">98.9%</span>
              <span className="block text-sm text-muted-foreground mt-2">Taux de livraison</span>
            </div>
            ...
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              ...
              <span className="block text-sm text-muted-foreground mt-2">Channels supported</span>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4 transition-all duration-1000 delay-300 ${
          ...
          {regions.map((region, index) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground">{region.nodes} countries</span>
            </div>
          ))}
        </div>
      </div>
