...
import { MessageEngine } from "@/lib/message";
import { toNewRecipient, toDashboardRecipient } from "@/lib/integration/dashboard-integration";
import { useNotifications } from "@/lib/hooks";
import type { ImportRow } from "@/lib/types";

type WizardView = "form" | "summary" | "sending" | "done" | "paused";
...
export function NewCampaignPanelV2() {
  const { addCampaign, setSection, campaignManager, queueManager } = useDashboard();
  const { campaignCreated, campaignCompleted, campaignPaused, campaignResumed } = useNotifications();
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Convert ImportRow to Recipient format
      const convertedRecipients = ImportService.toRecipients(result.rows).map(r => toDashboardRecipient(r));
      setRecipients(convertedRecipients.length ? convertedRecipients : [emptyRecipient()]);
  ...
  const startSending = useCallback(() => {
    ...
    const newRecipients = validRecipients.map(r => toNewRecipient(r));
    const campaign = campaignManager.createCampaign(
      `Campagne ${CHANNEL_LABELS[channel!]} — ${new Date().toLocaleDateString("fr-FR")}`,
      channel!,
      message,
      newRecipients,
    ...
    campaignCreated(campaign.nom);

    // Start the campaign
    campaignManager.startCampaign(campaign.id);

    // Simulate progress for UI (in real implementation, this would come from CampaignManager callbacks)
    let sent = 0;
    let failed = 0;
      ...
      if (sent + failed >= total) {
        ...
        setTimeout(() => {
          addCampaign({
            ...
            message,
            createdAt: campaign.dateCreation.toISOString(),
          });
          campaignCompleted(campaign.nom, sent);
          setView("done");
        }, 600);
        return;
      }
  ...
  }, [validRecipients, channel, subject, message, addCampaign, campaignManager, campaignCreated, campaignCompleted]);

  const pauseSending = useCallback(() => {
    if (currentCampaignId) {
      campaignManager.pauseCampaign(currentCampaignId);
      setView("paused");
      campaignPaused(`Campagne ${CHANNEL_LABELS[channel!]}`);
    }
  }, [currentCampaignId, campaignManager, channel, campaignPaused]);

  const resumeSending = useCallback(() => {
    if (currentCampaignId) {
      campaignManager.resumeCampaign(currentCampaignId);
      setView("sending");
      campaignResumed(`Campagne ${CHANNEL_LABELS[channel!]}`);
    }
  }, [currentCampaignId, campaignManager, channel, campaignResumed]);
  ...
  if (view === "sending" || view === "done" || view === "paused") {
    return (
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
          {view === "done" ? (
            ...
            <>
              ...
              <h2 className="font-display text-3xl">Campagne en pause</h2>
              <p className="text-muted-foreground">
                {sendProgress.sent} message(s) envoyé(s), {sendProgress.pending} en attente.
              </p>
              <div className="flex gap-4 justify-center pt-4">
                <Button variant="outline" className="rounded-full" onClick={() => { setView("form"); setStep(0); setChannel(null); setRecipients([emptyRecipient()]); setMessage(""); setSubject(""); }}>
                  Annuler
                </Button>
                <Button className="rounded-full bg-foreground text-background" onClick={resumeSending}>
            ...
            <>
              ...
              <Button variant="outline" className="rounded-full mt-4" onClick={pauseSending}>
        ...
        >
                ...
                {importResult && (
                  <div className="border border-border p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div><p className="text-muted-foreground">Lignes</p><p className="font-mono">{importResult.totalRows}</p></div>
                      <div><p className="text-muted-foreground">Valides</p><p className="font-mono text-green-400">{importResult.validRows}</p></div>
                      <div><p className="text-muted-foreground">Invalides</p><p className="font-mono text-destructive">{importResult.invalidRows}</p></div>
                    </div>
          ...
          {step === 3 && (
            <div className="border border-border bg-card p-6 space-y-5 max-w-2xl">
              ...
              <p className="text-xs text-muted-foreground">Variables disponibles : <code className="font-mono">{`{{nom}}`}</code>, <code className="font-mono">{`{{prénom}}`}</code>, <code className="font-mono">{`{{entreprise}}`}</code></p>
          ...
          {step === 4 && (
            <div className="border border-border bg-card p-6 max-w-2xl space-y-4">
              ...
              {channel === "email" && subject && (
                <div><p className="text-xs text-muted-foreground mb-1">Objet</p><p className="font-medium">{MessageEngine.replaceVariables(subject, toNewRecipient(previewRecipient))}</p></div>
              )}
              <div className="border border-border p-4 bg-secondary/30 whitespace-pre-wrap text-sm">
                {MessageEngine.replaceVariables(message, toNewRecipient(previewRecipient))}
                {channel === "email" && emailSender.signature && (
